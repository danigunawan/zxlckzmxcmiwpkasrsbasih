   ...
   while  ($cek0 = mysqli_fetch_array($query0)){
    ...
    $sisa_kredit = angkadoang($_POST['kredit']);
      $pembayaran = angkadoang($_POST['pembayaran']);
            ...
            $stmt = $db->prepare("UPDATE penjualan SET penjamin = ?, apoteker = ?, kode_gudang = ?, total = ?, tanggal = ?, jam = ?, user = ?, sales = ?, status = 'Lunas', potongan = ?, tax = ?, sisa = ?, cara_bayar = ?, tunai = ?, status_jual_awal = 'Tunai', keterangan = ?, ppn = ?,jenis_penjualan = ?,biaya_admin = ?, petugas_edit = ?, waktu_edit = ? WHERE no_faktur = ?");
            
            
            // hubungkan "data" dengan prepared statements
            $stmt->bind_param("sssissssiiisisssisss", 
            $penjamin,$apoteker, $kode_gudang, $total, $tanggal_sekarang, $jam_sekarang, $id_kasir, $petugas_kasir, $potongan, $tax, $sisa, $cara_bayar, $pembayaran, $keterangan, $ppn_input,$jenis_penjualan,$biaya_admin,$petugas_edit,$waktu,$no_faktur);
...
//POTONGAN
        $insert_juranl = $db->query("INSERT INTO jurnal_trans (nomor_jurnal,waktu_jurnal,keterangan_jurnal,kode_akun_jurnal,debit,kredit,jenis_transaksi,no_faktur,approved,user_buat) VALUES ('".no_jurnal()."', '$tanggal_sekarang $jam_sekarang', 'Penjualan Apotek Tunai - $ambil_kode_pelanggan[nama_pelanggan]', '$ambil_setting[potongan_jual]', '$potongan', '0', 'Penjualan', '$no_faktur','1', '$user')");
              ...
              
            {
              ...
              $stmt2 = $db->prepare("UPDATE penjualan SET penjamin = ?, apoteker = ?, kode_gudang = ?,total = ?, tanggal = ?, jam = ?,  user = ?, sales = ?, status = 'Piutang', potongan = ?, tax = ?, kredit = ?, cara_bayar = ?, tunai = ?, status_jual_awal = 'Kredit', keterangan = ?, ppn = ?,jenis_penjualan = ?,tanggal_jt = ?,biaya_admin = ?, petugas_edit = ?, waktu_edit = ?  WHERE no_faktur = ?");
            
            
            // hubungkan "data" dengan prepared statements
            $stmt2->bind_param("sssissssiiisissssisss", 
            $penjamin,$apoteker, $kode_gudang, $total, $tanggal_sekarang, $jam_sekarang, $id_kasir, $petugas_kasir, $potongan, $tax, $sisa_kredit, $cara_bayar, $pembayaran, $keterangan, $ppn_input,$jenis_penjualan,$tanggal_jt,$biaya_admin,$petugas_edit,$waktu,$no_faktur);
